...
import './toPromise.js';
import {defaultAlertMsg} from './config';
...
function httpServer(opt, callBack) {
  ...
  let timer = null;
  timer = setTimeout(() => { utils.toast('加载中...', 1500, 'loading') }, 500);
  return wx.request({
      url: url,
      data: data,
      header: header,
  ...
  .then((res) => {
      ...
      if (res && res.statusCode == 200 && res.data && res.data.success) {
          ...
          throw { message: res.data.msg.toString() };
      } else {
          console.log(res);
          throw {};
      }
      ...
      if (err.message) {
          ...
          setTimeout(() => { utils.toast('哎呦，网络开小差了≧﹏≦') }, 0)
